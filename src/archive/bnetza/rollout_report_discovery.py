...
Usage:
    rollout_report_discovery.py [--check-update] [--verbose]
    rollout_report_discovery.py (-h | --help)
...
Examples:
    ...
    python rollout_report_discovery.py --check-update

    # Discover and store new reports in database
    python rollout_report_discovery.py

    # Show detailed information
    python rollout_report_discovery.py --verbose
...
# Setup module logger
logger = logging.getLogger("report_discovery")

# Constants for magic value compliance
MAX_URL_LENGTH = 80
MAX_ETAG_LENGTH = 20
MAX_STRING_LENGTH = 60
MAX_FIRST_PART_LENGTH = 1
MAX_LAST_PART_LENGTH = 1


def _shorten_string(text: str, max_length: int = MAX_STRING_LENGTH) -> str:
    ...
    Returns:
        Shortened string with "..." if truncated
    """
...
def _shorten_url(url: str) -> str:
    ...
    Returns:
        Shortened URL focusing on the important parts
    """
    ...
    if "/" in url:
        filename = url.split("/")[-MAX_LAST_PART_LENGTH]
        if "?" in filename:
            filename = filename.split("?")[0]
        domain = (
            url.split("//")[MAX_FIRST_PART_LENGTH].split("/")[0]
            if "//" in url
            else url.split("/")[0]
        )
        return f"{domain}/.../{filename}"

    return _shorten_string(url, MAX_URL_LENGTH)
...
class BNetzAReportDiscovery:
    ...
    def store_report_in_database(
        ...
        """
        try:
            # Check if this report already exists by filename and URL
            check_sql = """
                SELECT id, status FROM rollout_update_logs
                WHERE excel_filename = :filename
            ...
            existing = self._get_db_session().execute(
                text(check_sql),
                {
                    "filename": str(report_info["filename"]),
                    "article_url": str(report_info["url"]),
                },
            )
            existing_row = existing.fetchone()
            if existing_row:
                existing_id = int(existing_row[0])
            ...
            # Insert new report record using actual schema (excel_file_hash will be set later after processing)
            insert_sql = """
            ...
            params = {
                ...
                "excel_file_hash": None,  # Initially NULL, will be set after successful processing
                "report_quarter": quarter,
                "report_year": year,
            ...
            if "duplicate key value violates unique constraint" in str(e):
                # Try to find the existing record by filename as fallback
                logger.warning(
                    f"Duplicate detected, searching for existing record: {report_info.get('filename')}"
                )
                try:
                    fallback_sql = """
                        SELECT id, status FROM rollout_update_logs
                        ...
                        LIMIT 1
                    """
                    result = self._get_db_session().execute(
                        text(fallback_sql), {"filename": str(report_info["filename"])}
                    )
                    row = result.fetchone()
                    if row:
                        ...
                        logger.info(
                            f"Found existing report by filename: ID {existing_id}, Status: {existing_status}"
                        )
                        return existing_id
                except Exception as fallback_error:
                    logger.error(f"Fallback lookup failed: {fallback_error}")

                # If we can't find it, re-raise the original error
                logger.error(f"Could not resolve duplicate, original error: {e}")
                raise
            else:
                # For non-duplicate errors, rollback and re-raise
    ...
    def set_report_file_hash(
        self, record_id: int, file_hash: str, status: str = "completed"
    ) -> None:
        """Set the excel_file_hash after successful file processing.

        Args:
            record_id: Database ID of the report
            file_hash: SHA-256 hash of the processed Excel file
            status: Final status to set ('completed' by default)
        """
        try:
            update_sql = """
                UPDATE rollout_update_logs
                SET excel_file_hash = :file_hash,
                    status = :status,
                    processing_timestamp = :processing_timestamp,
                    notes = COALESCE(notes, '') || ' | File hash set after successful processing'
                WHERE id = :record_id
            """
            ...
            self._get_db_session().commit()
            logger.info(
                f"Set file hash for report {record_id} and updated status to: {status}"
            )

        except Exception as e:
            self._get_db_session().rollback()
            logger.error(f"Failed to set report file hash: {e}")
            raise
    ...
    def _log_head_metadata(self, url: str, metadata: dict[str, str]) -> None:
        ...
        logger.info(f"   🔗 URL: {_shorten_url(url)}")

        if metadata.get("etag"):
            etag_short = _shorten_string(metadata["etag"], MAX_ETAG_LENGTH)
            logger.info(f"   🏷️  ETag: {etag_short}")
        else:
            ...
            try:
                ...
                if row:
                    ...
                    if current_etag and stored_etag == current_etag:
                        ...
                        logger.info(
                            f"   🏷️  Database ETag: {_shorten_string(stored_etag, MAX_ETAG_LENGTH)}"
                        )
                        logger.info(
                            f"   🏷️  Current ETag:  {_shorten_string(current_etag, MAX_ETAG_LENGTH)}"
                        )
    ...
    def download_report(
        ...
        try:
            if save_path is None:
                # Extract filename from URL
                filename = urlparse(report_url).path.split("/")[-MAX_LAST_PART_LENGTH]
            ...
            # Download the file
            logger.info(f"Downloading report from: {_shorten_url(report_url)}")
            response = requests.get(
                report_url,
                headers={"User-Agent": BNetzAConfig.USER_AGENT},
                timeout=BNetzAConfig.REQUEST_TIMEOUT,
            ...
            try:
                ...
                if row and row[0] == current_etag and current_etag:
                    ...
                    logger.info(
                        f"   🏷️  Database ETag: {_shorten_string(row[0], MAX_ETAG_LENGTH)}"
                    )
                    logger.info(
                        f"   🏷️  Current ETag:  {_shorten_string(current_etag, MAX_ETAG_LENGTH)}"
                    )
                    ...
                    if row and current_etag:
                        ...
                        logger.info(
                            f"   🏷️  Database ETag: {_shorten_string(row[0] or '', MAX_ETAG_LENGTH)}"
                        )
                        logger.info(
                            f"   🏷️  Current ETag:  {_shorten_string(current_etag, MAX_ETAG_LENGTH)}"
                        )
                        ...
                        if current_etag:
                            logger.info(
                                f"   🏷️  Current ETag:  {_shorten_string(current_etag, 20)}"
                            )
