"""Convert TSV to PostgreSQL VALUES list with duplicate checks."""
...
def quote(val: str, is_number: bool = False) -> str:
    """Quote value for SQL, handling NULL and escaping."""
    val = val.strip()
    if val == "":
        return "NULL"
    if is_number:
...
def process_tsv(file_path: str) -> None:
    """Process TSV file and generate PostgreSQL INSERT statement."""
    ...
    with Path(file_path).open(encoding="utf-8", newline="") as f:
        ...
        for line_num, row in enumerate(reader, start=1):
            if len(row) != EXPECTED_COLUMNS:
                print(
                    f"⚠️  Zeile {line_num}: erwartet {EXPECTED_COLUMNS} Spalten, gefunden {len(row)} - übersprungen",
                    file=sys.stderr,
                )
                continue
            ...
            if not betrnr.isdigit():
                print(
                    f"⚠️  Zeile {line_num}: ungültiger betrnr '{betrnr}' - übersprungen",
                    file=sys.stderr,
                )
                continue
            ...
            if betrnr in seen_betrnr:
                ...
                print(
                    f"⚠️  Zeile {line_num}: doppelter betrnr '{betrnr}' (bereits in Zeile {first_line}) - übersprungen",
                    file=sys.stderr,
                )
                continue
            ...
            if company_key in seen_company:
                ...
                print(
                    f"⚠️  Zeile {line_num}: doppelte company '{company}' (bereits in Zeile {first_line}) - wird trotzdem übernommen",
                    file=sys.stderr,
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Convert TSV to PostgreSQL VALUES list with duplicate checks."
    )
